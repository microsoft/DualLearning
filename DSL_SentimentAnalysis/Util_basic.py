...
def itemlist_multiple(tparams_list, removeEmb=False):
    if removeEmb:
        return [vv for tparams in tparams_list for kk, vv in tparams.iteritems() if 'Wemb' not in kk]
    else:
        return [vv for tparams in tparams_list for kk, vv in tparams.iteritems()]
...
'''
def prepare_data_x(seqs_x, maxlen=None, pad_eos = True):
    # x: a list of sentences
    lengths_x = [len(s) for s in seqs_x]
...
def adadelta_joint(lr, tparams_list, grads_list, inp, cost):
    zipped_grads = [theano.shared(p.get_value() * numpy.float32(0.), name='%d_%s_grad' % (paraId, k))
                    for tparams in tparams_list for paraId, (k, p) in enumerate(tparams.iteritems())]
    running_up2 = [theano.shared(p.get_value() * numpy.float32(0.), name='%d_%s_rup2' % (paraId, k))
                   for tparams in tparams_list for paraId, (k, p) in enumerate(tparams.iteritems())]
    running_grads2 = [theano.shared(p.get_value() * numpy.float32(0.),name='%d_%s_rgrad2' % (paraId, k))
                      for tparams in tparams_list for paraId, (k, p) in enumerate(tparams.iteritems())]

    grads = []
    map(lambda gg: grads.extend(gg), grads_list)
    ...
    ru2up = [(ru2, 0.95 * ru2 + 0.05 * (ud ** 2))
             for ru2, ud in zip(running_up2, updir)]
    param_up = [(p, p + lr * ud) for p, ud in zip(itemlist_multiple(tparams_list), updir)]
...
def nesterov(lr, tparams, grads, inps, outs, useRNNonly=False):
    '''
    From https://github.com/lisa-lab/pylearn2/pull/136#issuecomment-10381617, we use the following formular:
    v_t = mu * v_t-1 - lr * gradient_f(params_t)
    params_t = params_t-1 + mu**2 * v_t-1 - (1+mu) * lr * gradient_f(params_t-1)
    '''
    mu = 0.9
    if useRNNonly:
        gshared = [theano.shared(p.get_value() * 0., name='%s_grad' % k) for k, p in tparams.iteritems() if 'ff_' not in k]
        vshared = [theano.shared(p.get_value() * 0., name='%s_shared_v' % k) for k, p in tparams.iteritems() if 'ff_' not in k]
    else:
        gshared = [theano.shared(p.get_value() * 0., name='%s_grad' % k) for k, p in tparams.iteritems()]
        vshared = [theano.shared(p.get_value() * 0., name='%s_shared_v' % k) for k, p in tparams.iteritems()]
    gsup = [(gs, g) for gs, g in zip(gshared, grads)]

    f_grad_shared = theano.function(inps, outs, updates=gsup, profile=profile)
    updates = []
    if useRNNonly:
        for w, v, g in zip(itemlist_RNNpart(tparams), vshared, gshared):
            updates.append((v, mu * v - lr * g))
            updates.append((w, w + mu * mu * v - (1 + mu) * lr * g))
    else:
        for w, v, g in zip(itemlist(tparams), vshared, gshared):
            updates.append((v, mu * v - lr * g))
            updates.append((w, w + mu * mu * v - (1 + mu) * lr * g))
    f_update = theano.function([lr], [], updates=updates, profile=profile)
    return f_grad_shared, f_update
    
# We can use different grad_clip for different models 
def sgd_joint2(lr, tparams_list, grads_list, inps, cost):
    gshared = [theano.shared(p.get_value() * 0., name='%d_%s_grad' % (paraId, k))
               for tparams in tparams_list for paraId, (k, p) in enumerate(tparams.iteritems())]
    grads = []
    map(lambda gg: grads.extend(gg), grads_list)
    gsup = [(gs, g) for gs, g in zip(gshared, grads)]
    ...
    pup = [(p, p - lr * g) for p, g in zip(itemlist_multiple(tparams_list), gshared)]
    f_update = theano.function([lr], [], updates=pup, profile=profile)

    return f_grad_shared, f_update

def sgd_joint(lr_list, tparams_list, grads_list, inps, costs, FreezeEmb=False):
    if FreezeEmb:
        gshared_list = [ [theano.shared(p.get_value() * 0.) for (k, p) in tparams.iteritems() if 'Wemb' not in k]
                          for tparams in tparams_list]
    else:
        gshared_list = [ [theano.shared(p.get_value() * 0.) for (_, p) in tparams.iteritems()]
                          for tparams in tparams_list]

    gsup = [(gs, g) for gshared, grads in zip(gshared_list, grads_list) for gs, g in zip(gshared, grads)]
    f_grad_shared = theano.function(inps, costs, updates=gsup, profile=profile)


    if FreezeEmb:
        pup = [(p, p - lr * g)
               for (lr, tparams, gshared) in zip(lr_list, tparams_list, gshared_list)
               for (p,g) in zip(itemlist_NoEmb(tparams), gshared)]
    else:    
        pup = [(p, p - lr * g)
               for (lr, tparams, gshared) in zip(lr_list, tparams_list, gshared_list)
               for (p,g) in zip(tparams.values(), gshared)]

    f_update = theano.function(lr_list, [], updates=pup, profile=profile)

    return f_grad_shared, f_update


def sgd_joint_FreezeEmb(lr, tparams_list, grads_list, inps, cost):
    gshared = [theano.shared(p.get_value() * 0., name='%d_%s_grad' % (paraId, k))
               for tparams in tparams_list for paraId, (k, p) in enumerate(tparams.iteritems()) if 'Wemb' not in k]
    ...
    pup = [(p, p - lr * g) for p, g in zip(itemlist_multiple(tparams_list, True), gshared)]
    f_update = theano.function([lr], [], updates=pup, profile=profile)
...
def randomSample_batch(f_init, f_next, x, x_mask, gpulock, k=16, maxlen=50):
    batch_size = x.shape[1]
    sample = [ [] for _ in xrange(batch_size)]
    lives_k = [k for _ in xrange(batch_size)]
    live_samples = [[ [] for _ in xrange(k) ] for _ in xrange(batch_size)]
    ...
    next_state, ctx0 = ret[0], ret[1]
    next_state = numpy.repeat(next_state, k, axis=0)
    next_w = numpy.array([-1] * batch_size * k).astype('int64')  # indicator for the first target word (bos target)

    for _ in xrange(maxlen):
        ctx = numpy.repeat(ctx0, lives_k, axis=1)
        x_extend_masks = numpy.repeat(x_mask, lives_k, axis=1)
        ...
        next_w_sampled, next_state = ret[1], ret[2]

        uidx = 0
        for jj in xrange(batch_size):
            for ii in xrange(len(live_samples[jj])):
                live_samples[jj][ii].append(next_w_sampled[uidx])
                uidx += 1
            for ii in xrange(len(live_samples[jj])-1, -1, -1):
                if live_samples[jj][ii][-1] == 0:
                    sample[jj].append(live_samples[jj][ii])
                    del live_samples[jj][ii]
                    lives_k[jj] -= 1

        if numpy.array(lives_k).sum() > 0:
            rowToBeRemoved = numpy.where(next_w_sampled == 0)[0]
            next_w = numpy.delete(next_w_sampled, rowToBeRemoved, axis=0)
            next_state = numpy.delete(next_state, rowToBeRemoved, axis=0)
        else:
            break
    ...
    for jj in xrange(batch_size):
        if lives_k[jj] > 0:
            for idx in xrange(lives_k[jj]):
                sample[jj].append(live_samples[jj][idx])
